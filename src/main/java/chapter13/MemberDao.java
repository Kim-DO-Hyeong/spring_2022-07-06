...
public class MemberDao {
	...
	public Member selectByMemberNumber(long memberNumber) {
		List<Member> result = jdbcTemplate.query("SELECT * FROM member WHERE memberNumber =? ", new MemberMapper(),
				memberNumber);
	...
	public List<Member> selectByRegDate(LocalDateTime from, LocalDateTime to) {
		List<Member> results = jdbcTemplate.query(
				"SELECT * FROM member WHERE regDate BETWEEN ? AND ? ORDER BY regDate DESC", new MemberMapper(), from,
				to);
		return results;
	}
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				PreparedStatement pstmt = con.prepareStatement(
						"INSERT INTO member(email,password,name,regDate) VALUES (?,?,?,?)", columnNames);
				pstmt.setString(1, member.getEmail());
				pstmt.setString(2, member.getPassword());
				pstmt.setString(3, member.getName());
				pstmt.setString(4, member.getRegisterDateTime().toString());
